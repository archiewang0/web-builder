...
import { ComponentIdEnums } from '@/builder/_components/sidebar/component-id-enums';
...
function RenderSchemaElement({ element }: { element: ElementSchema }) {
    ...
    switch (element.componentId) {
        ...
        case ComponentIdEnums.button:
            return (
                <button style={style} className="shadow-md transition-all hover:opacity-80 rounded px-4 py-2">
                    {element.content || '按鈕'}
                </button>
            );
